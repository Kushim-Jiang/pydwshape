...
/* NOTE (port): this is used as the UNIVERSAL joining shaper for every script,
 * not just Arabic/Syriac. Arabic-style joining is driven purely by each
 * glyph's Unicode joining type from `arabic_shaping_table` (generated from
 * UCD ArabicShaping.txt, which includes Arabic, Syriac, Mongolian, N'Ko, ...).
 * Scripts/glyphs whose joining type is U or T never receive a positional
 * action, so non-joining scripts (Latin, Hebrew, digits, punctuation) are
 * unaffected. This mirrors HarfBuzz: one joining shaper, no per-script
 * branches; the caller only picks which OpenType script table of the font to
 * use (shape.c / shape_set_shaper). */
