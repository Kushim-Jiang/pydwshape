...
{
    /* Universal joining shaper: no per-script branches. Arabic, Syriac,
     * Mongolian, N'Ko, ... are all driven by each glyph's Unicode joining
     * type (ArabicShaping.txt); glyphs that do not join (Latin, digits,
     * punctuation...) get no positional action and are unaffected. */
